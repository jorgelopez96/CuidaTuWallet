...
const ProfilePage = () => {
  ...
  useEffect(() => {
    if (profile) {
      setForm({ name: profile.name || '', birthdate: profile.birthdate || '' })
    }
  }, [profile])
  ...
  return (
    <div className="p-4 md:p-6 max-w-lg mx-auto">
      ...
      <Card className="flex flex-col items-center py-8 mb-4">
        ...
        <p className="dark:text-slate-400 text-slate-500 text-sm mt-1">{profile?.email}</p>
        {profile?.birthdate && (
          <p className="dark:text-slate-500 text-slate-400 text-xs mt-1">
            Nacimiento: {new Date(profile.birthdate).toLocaleDateString('es-AR', { day: '2-digit', month: 'long', year: 'numeric' })}
          </p>
        )}
        {profile?.createdAt && (
          <p className="dark:text-slate-500 text-slate-400 text-xs mt-1">
        ...
        {isEditing ? (
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center py-2 border-b dark:border-white/10 border-slate-100">
              ...
              <span className="text-sm dark:text-white text-slate-900 font-medium">{profile?.name || '—'}</span>
            </div>
            <div className="flex justify-between items-center py-2 border-b dark:border-white/10 border-slate-100">
              <span className="text-sm dark:text-slate-400 text-slate-500">Email</span>
              <span className="text-sm dark:text-white text-slate-900 font-medium">{profile?.email || '—'}</span>
            </div>
